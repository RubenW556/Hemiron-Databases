...
describe('DatabasesController', () => {
  ...
  const mockResponse = {
    status: jest.fn((x) => x),
    locals: { userMakingRequest: { id: '448098df-b9dd-4ec2-af9f-b23459b203a1' } }
  ...
  const mockDatabasesService = {
    ...
    findAllForUser: jest.fn(async (): Promise<Database[]> => [validDatabaseRecord]),
    insert: jest.fn(async function (databaseDto: CreateDatabaseDto, userMakingRequest: UserMakingRequest): Promise<ReturnDatabase> {
      return { database_id: '836b450a-3720-48ab-ade5-97eb692737e2', databaseName: databaseDto.name, user_id: userMakingRequest.id } as ReturnDatabase;
    }),
    update: jest.fn(async function (updateDatabaseDto: UpdateDatabaseDto): Promise<UpdateResult> {
  ...
  describe('create', () => {
    const createDatabaseDto: CreateDatabaseDto = { name: 'database-to-create', type: 'postgres' };
    ...
    it('should call insert on DatabasesService', async () => {
      ...
      expect(spy).toHaveBeenCalledWith(createDatabaseDto, mockResponse.locals.userMakingRequest);
    });
    ...
    it('should add permissions for created database', async () => {
      ...
      await controller.create(mockResponse, createDatabaseDto);
      expect(spy).toHaveBeenCalledWith(expect.any(String), mockResponse.locals.userMakingRequest.id);
    });

  });

  describe('update', () => {
    const updateDatabaseDto: UpdateDatabaseDto = { id: validDatabaseRecord.id, name: 'database-with-updated-name' };
